#ifndef __OPERATORCONTROLLER
#define __OPERATORCONTROLLER
...
class OperatorController : public VisitController {
public:
 	int goalLevel;
	int negationLevel;
	int qgoalLevel;
        int qnegationLevel;
 	bool preconditionMode;
 	bool qfiedGoalMode;
	bool inLValue;
        VAL::Processor* proc;
        operator_* currentOp;
	qfied_goal* currentQFiedGoal;
	StringToInt opArgsMap; // Maps the name of a variable to the corresponding element in args
	OperatorController(VAL::Processor* proc_);
	void processRevMsg(proposition* p);
	void assignParameterIndex(const string& name);
	void considerOptimizations(simple_goal* g);
	void considerQFiedGoalOptimizations(simple_goal* g);
	...
	virtual void visit_uminus_expression(uminus_expression *) {assert(false);};
