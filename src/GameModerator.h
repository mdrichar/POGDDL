...
public:
  ...
  int chooseMove(PlayerType pt, Processor* p, VecInt& canDo, unsigned pid, VecVecVecKey& obs, WorldState& initState, int nSamples, int oppSamples);
  int chooseMCTSMove(Processor* p, unsigned pid, VecVecVecKey& obs, SetVecInt& infoSet, WorldState& initState);
  int chooseInferenceMove(Processor* p, unsigned pid, VecVecVecKey& obs, SetVecInt& infoset, const VecInt& legalOptions, WorldState& initState, int nSamples, int oppSamples);
  int chooseHumanMove(Processor* p, const VecInt& legalActions, WorldState& initState);
  void analyzeInfosetFailure(const VecInt& gameHistory, WorldState initState, WorldState current, Processor* p, unsigned k);
  ...
  int getDecisionMCTS(const VecInt& legalOptions, const VecVecFloat& vvf, VecFloat& sums);
  int chooseMoveMCTS(Processor* p, const VecVecVecKey& obs, const SetVecInt& infoset, const VecInt& legalOptions, const WorldState& initState, int nSamples);
};
