#ifndef __GOALCONTROLLER
#define __GOALCONTROLLER
...
class GoalVisitController : public VisitController {
public:
  ...
  int operatorNumber;
	...
	VecStr labels;
	VecStr effects;
	VecInt preconditionNums;
        VecInt effectNums;

        const StringToInt& params;
	const StringToInt& objectTbl;
        const StringToBounds& offsets;
        const PredicateMultTable& mults; 
        VAL::Processor* proc;
	GoalVisitController(const StringToInt& params_, const StringToInt& objects_, const StringToBounds& offsets_,
          const PredicateMultTable& mults_, VAL::Processor* proc_) 
          : params(params_), objectTbl(objects_), offsets(offsets_), mults(mults_), proc(proc_) {};
	virtual ~GoalVisitController() {};
  int getIndex(const StringToBounds& offsets, const PredicateMultTable& mults, const string& name, const VecInt& args);
  int encodeFact(const proposition* p);
	...
	virtual void visit_recipient(recipient *) ;
	...
	virtual void visit_recipient(const recipient *r) {visit_recipient(const_cast<recipient*>(r));};
