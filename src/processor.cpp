...
void Processor::processOperators() {
	...
	for (StringToInt::const_iterator itr = revHeadTbl.begin(); itr != revHeadTbl.end(); ++itr) {
		...
		for (unsigned j = 0; j < revParamTypeTbl[itr->second].size(); j++) {
			if (superVerbose)
				cout << "Param " << j << " has id " << revParamTypeTbl[itr->second][j] << " and card "
						<< revParamTypeCardTbl[itr->second][j] << "\n";
		}
	}
}
...
string Processor::observationToString(VecInt & currentObs) {
	...
	VecInt & obsParamTypeIds = this->revParamTypeTbl[observationHeadId];
...
string Processor::getObjectName(int typeId, int objectId) {
	assert(typeId < objectTblByTypeId.size());
	assert(objectId < objectTblByTypeId[typeId].size());
