...
// Display a data member that is a parse_category
#define DFIELD(NAME, OUT) indento(ind,OUT); if (NAME != NULL) NAME->write2(OUT,ind); else OUT << "(NULL)";  OUT << " ";
...
// Output a data member that is not a parse_category
#define DLEAF(NAME,OUT) indent(ind); OUT << NAME << ":";

//class std::ostream;
extern void indent(int ind);
extern void indento(int ind, std::ostream& o);
