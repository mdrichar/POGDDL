...
%type <t_gain_def> c_gain_def
%type <t_observation> c_observation
%type <t_recipients> c_recipients
%type <t_recipient> c_recipient
...
%token <punct> OPEN_BRAC CLOSE_BRAC 
       ...
       AND OR EXISTS FORALL IMPLY NOT WHEN WHENEVER EITHER OBSERVE
       PROBLEM FORDOMAIN INITIALLY
       OBJECTS GOALS EQ LENGTH SERIAL PARALLEL METRIC GAIN
       MINIMIZE MAXIMIZE
...
c_domain_require_def : 
   OPEN_BRAC REQS c_reqs CLOSE_BRAC
	...
	$$=$3;
	Counter::resetVarListId(); // Need to do this somewhere before predicate list
    }
...
c_pred_decl :
    OPEN_BRAC c_new_pred_symbol c_typed_var_list CLOSE_BRAC
       {$$= new pred_decl($2,$3,current_analysis->var_tab_stack.pop()); Counter::resetVarListId();}
...
// recipient of a observation
c_recipients :
    c_recipients c_recipient
	{$$ = $1; $$->push_back($2);}
|  /* Empty */ {$$= new recipients;}
;
...
c_var_symbol :
    NAME { $$= current_analysis->var_tab_stack.symbol_get($1); $$->setIndex(); /*Keeps track of this var's order in the list*/ delete [] $1; }
;
...
|	OPEN_BRAC WITHIN c_number c_goal_descriptor CLOSE_BRAC
		{$$ = new constraint_goal(E_WITHIN,$4,NULL,$3->floating_value(),0.0);delete $3;}
...
|   OPEN_BRAC ALWAYSWITHIN c_number c_goal_descriptor c_goal_descriptor CLOSE_BRAC
		{$$ = new constraint_goal(E_ALWAYSWITHIN,$5,$4,$3->floating_value(),0.0);delete $3;}
| 	OPEN_BRAC HOLDDURING c_number c_number c_goal_descriptor CLOSE_BRAC
		{$$ = new constraint_goal(E_HOLDDURING,$5,NULL,$4->floating_value(),$3->floating_value());delete $3;delete $4;}
|   OPEN_BRAC HOLDAFTER c_number c_goal_descriptor CLOSE_BRAC
		{$$ = new constraint_goal(E_HOLDAFTER,$4,NULL,0.0,$3->floating_value());delete $3;}
;
