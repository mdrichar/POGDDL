...
struct Bounds
{
  ...
  unsigned upper;
  Bounds() { assert (false);}
  Bounds(unsigned l, unsigned u) : lower(l), upper(u) {}
};
...
typedef set<VecInt> SetVecInt;
typedef vector<SetVecInt> VecSetVecInt;
typedef map< string, SetStr > TypeTable;
typedef pair< string, SetStr > TypeTableEntry;
typedef pair< string, VecStr > PredicateTableEntry;
typedef pair< string, VecInt > PredicateMultTableEntry;
typedef pair< string, int > StringToIntEntry;
typedef pair< string, Bounds > StringToBoundsEntry;
typedef map< string, VecStr > PredicateTable;
typedef map< string, VecInt > PredicateMultTable;
typedef map< string, int > ObjectToInt;
typedef map< string, int > StringToInt;
typedef map< string, Bounds > StringToBounds;
...
//	string name;
//	VecInt args;
//	FunctionKey(const string& n, const VecInt& a) : name(n), args(a) {}
//	bool operator<(const FunctionKey& other) const;
//	string asString() const;
//		
//};
typedef vector<int> FunctionKey;
string asIntString(const FunctionKey& fk);
typedef vector<FunctionKey> VecKey;
typedef vector<VecKey> VecVecKey;
typedef vector<VecVecKey> VecVecVecKey;

//typedef long double NumScalar;
typedef float NumScalar;
typedef map<FunctionKey,NumScalar> FluentTable;
typedef vector< FluentTable > VecFluents;
typedef FluentTable::const_iterator FTitr;
typedef vector<float> VecPayoff;

class Counter {
public:
  static int varListId;
  static void resetVarListId();
  static int getVarListIdAndInc();
};
/*---------------------------------------------------------------------------
...
public:
    ...
    virtual void write2(ostream& o, int ind = 0) const { o << "pc";};
...
public:
    ...
    virtual void display(int ind) const;
    virtual void write(ostream & o) const;
    virtual void write2(ostream & o, int ind) const;
    virtual void visit(VisitController * v) const;
};
...
template<class pc>
void pc_list<pc>::write2(ostream & o, int ind = 0) const
{
	//o << "pclist ";
	for (typename pc_list<pc >::const_iterator i=_Base::begin(); i!=_Base::end(); ++i)
	{
...
public :
    ...
    virtual void write(ostream& o, int ind = 0) const
    {
        for (const_iterator i=_Base::begin(); i!=_Base::end(); ++i)
        {
		LEAF(i->first);
    ...
    virtual void write2(ostream& o, int ind = 0) const
    {
	o << "st: ";
        for (const_iterator i=_Base::begin(); i!=_Base::end(); ++i)
        {
...
public:
    ...
    virtual void display(int ind) const;
    virtual void write(ostream & o) const;
    virtual void write2(ostream & o, int ind = 0) const { o << "$" << name; };
	virtual void visit(VisitController * v) const;
...
public:
    ...
    void setIndex() {index = Counter::getVarListIdAndInc();}
...
public:
    ...
    virtual void write2(ostream & o, int ind) const;
    virtual void visit(VisitController * v) const;
    virtual unsigned getHeadId() const {return headId;}
    virtual void setHeadId(unsigned id) { headId = id;}
    const pred_symbol* getHead() const { return head; }
    const parameter_symbol_list* getArgs() const { return args; }
	void setArgs(const VecInt& argMap_in) { argMap = argMap_in; }
	int lastParamUsed() const;
};
...
public:
	...
	virtual const NumScalar floating_value() const = 0;
	};
...
public:
    ...
    const NumScalar floating_value() const {return static_cast<const NumScalar>(val);};
};
...
public:
	virtual void display(int ind) const;
	virtual void write(ostream & o) const;
    	virtual void write2(ostream & o, int ind = 0) const {o << "goal ";};
	virtual void visit(VisitController * v) const;
        virtual void setPolarity(bool p) {};
};
...
public:
	...
	virtual void display(int ind) const;
	virtual void write(ostream & o) const;
    	virtual void write2(ostream & o, int ind = 0) const {o << "cons_goal";};
	virtual void visit(VisitController * v) const;
...
public:
	...
	virtual void display(int ind) const;
	virtual void write(ostream & o) const;
    	virtual void write2(ostream & o, int ind = 0) const {o << "preference";};
	virtual void visit(VisitController * v) const;
};
...
public:
    ...
    void setPolarity(bool p) { plrty = p ? E_POS : E_NEG;};
    proposition * getProp() const {return prop;};
};
typedef vector< simple_goal* > VecGoal;
typedef vector< VecGoal > VecVecGoal;
...
public:
    SetInt prePos;
    SetInt preNeg;
    VecVecGoal prePosList;
    VecVecGoal preNegList;
    VecInt argMap;
    VecInt argMaxes;
...
public:
	...
	virtual void write2(ostream & o, int ind) const;
	virtual void visit(VisitController * v) const;
    goal_list* getGoals() { return goals; };
};
...
public:
    ...
    pc_list<observation*>    observation_effects;
...
class recipient : public effect
{
public:
	parameter_symbol_list* receivers;
	proposition* prop;

public:
	recipient(parameter_symbol_list* r, proposition* p) :receivers(r), prop(p)  {};
	~recipient() { delete receivers; delete prop;}
	virtual void visit(VisitController * v) const;
        virtual void write2(ostream & o, int ind) const;
};
typedef pc_list<recipient*> recipients;

class observation : public effect
{
public:
	recipients* receivers;
	StringToInt recipientMapper;

public:
	observation(recipients* r) :receivers(r) {};
	~observation() { delete receivers;}
	virtual void visit(VisitController * v) const;
        virtual void write2(ostream & o, int ind) const;
};
...
public:
	...
	virtual void add_to(operator_list * ops,derivations_list * dvs, gain_list* gains)
	{};
};
...
public:
    VecInt argMap;
    unsigned argSizeNeeded;
    gain_def(expression* e, 
//	      typed_symbol_list<var_symbol>* a, 
	      var_symbol_list* a, 
	      var_symbol_table* vt,
	      goal* c) :
	exp(e), args(a), var_tab(vt), cond(c)  {};
	NumScalar getPayoff() const {return payoff;};
 	void setPayoff(NumScalar p) {payoff = p;};
	expression* getExp() {return exp;};
        const var_symbol_list * getArgs() const {return args;};
	const goal* getCond() const {return cond;};
    	~gain_def() { delete args; delete var_tab; delete cond;};
        virtual void setArgSizeNeeded(unsigned v) { argSizeNeeded = v; };
        virtual unsigned getArgSizeNeeded() { return argSizeNeeded; };
    	virtual void display(int ind) const;
    	virtual void write(ostream & o) const;
    	virtual void visit(VisitController * v) const;
	virtual void add_to(operator_list* ops,derivations_list* drvs,gain_list* gains);
};
//typedef pc_list<gain_def*> gain_list;

class gain_list : public pc_list<gain_def*>
{
public:
...
public:
    ...
    virtual unsigned getHeadId() const {return headId;}
    virtual void setHeadId(unsigned id) { headId = id;}
    virtual int getVarIndex(const string& name) {
      StringToInt::const_iterator itr = argMapper.find(name);
      if (itr != argMapper.end()) return itr->second;
      return UVAL;
    }
...
public:
	...
	gain_list* get_gains() {return gains;};
};
...
public:
	...
	virtual void write(ostream & o) const;
	virtual void write2(ostream & o, unsigned ind) const;
	virtual void visit(VisitController * v) const;
...
public:
	...
	virtual var_symbol_table * buildGainTab() {return new var_symbol_table;};
...
public:
	...
	var_symbol_table * buildGainTab() {return varTabFactory->buildGainTab();};
    ...
    {
	o << "TYPES:\n" ;
	pddl_type_tab.write2(o,ind);		
	o << "\nCONSTANTS:\n";
	const_tab.write2(o,ind);
	o << "\nPREDICATE NAMES:\n";
	pred_tab.write2(o,ind);
	o << "\nOPERATORS:\n";
	op_tab.write2(o,ind);
	if (the_domain) {
		the_domain->write2(o,ind);
        }
    };
    virtual ~analysis() 
	{
