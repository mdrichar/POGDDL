...
string GameModerator::typeString(PlayerType pt)
{
  switch (pt) {
    ...
    case P_MCTS: return "MCTS";
    case P_RANDOM: return "RANDOM";
    case P_MCTS_LIMITED: return "LIMITED";
    case P_INFER: return "INFER";
    default: return "UNKNOWN";
  }
  return "IMPOSSIBLE";
}

VecPayoff GameModerator::playGame(int seed)
{
  DefaultGameLogger defaultLogger;
  return playGame(seed, defaultLogger);
}

VecPayoff GameModerator::playGame(int seed, GameLogger& gameLogger)
{
  gameLogger.reset();
  VecInt gameHistory(1,-1); // 0th action gets -1 as a dummy; first true action is gameHistory[1]
  ActionGraph::fluentHistory.resize(1);
  if (medverbose) cout << "Seed: " << seed << "\n";
  if (seed) {
    srand(seed);
#ifdef PRANDOM
  ...
  while (true) {
    ...
    if (players[whoseTurn] == P_RANDOM || canDo.size() == 1) { // Don't mess with any sampling if it's a forced move (i.e., canDo.size() == 1); just do it
      ...
      cout << proc->getFormattedState(current) << std::endl;
	...
	if (gmverbose) cout << "WT: " << whoseTurn << "\n";
      chosenAction = chooseMove(players[whoseTurn], proc, canDo, whoseTurn, proc->kb, initState, GameModerator::manySamples, GameModerator::fewSamples);
    }
    if (chosenAction == -1) { // Means that information set generation failed
      analyzeInfosetFailure(gameHistory,initState,current,proc,whoseTurn);
    }
    ...
    if (medverbose || showApplying) cout << "(" << moveNumber << ") Applying: " << chosenAction << " " << proc->operatorIndexToString(chosenAction) << " chosen by " << whoseTurn << " who is " << typeString(players[whoseTurn]) << " to state: \n";
    moveNumber++;
    //getchar();
...
int GameModerator::chooseMove(PlayerType pt, Processor* p, VecInt& legalOptions, unsigned pid, VecVecVecKey& obs, WorldState& initState, int nSamples, int oppSamples)
{
  ...
  if (gmverbose) cout << "CHOOSE MOVE END INFOSET GENERATION\n";
  if (infoset.empty()) {
...
{
  if (gmverbose) cout << "ArgMax: " << vf.size() << "\n";
  assert (!vf.empty());
  int result = 0;
  float vmax = vf[0];
...
{
  if (gmverbose) cout << "MCTS EXPLORE: " << nSamples << "\n";
  assert (nSamples > 0);
  if (nSamples < 0) nSamples = (int)Node::nSamples;
  ...
  if (gmverbose) cout << "Running MCTS on active player: " << current.getWhoseTurn() << "\n";
  root.initializeRoot(current);
    if (root.terminal) { // \TODO: Maybe just assert !root.terminal
      if (gmverbose) cout << "INITIAL STATE\n";
      if (gmverbose) cout << proc->printPartialState(initState) << "\n";
      if (gmverbose) cout << "Root zeroed on initialization\n" ;
      if (gmverbose) cout << proc->printPartialState(Node::nodeVec()[0].ws) << "\n";
  ...
  for (int s = 0; s < nSamples; s++) {
    if (gmverbose || false) cout << "Sample #" << s << "\n";
...
VecVecFloat GameModerator::getDecisionMatrix(const SetVecInt& infoset, const WorldState& initState, Processor* p, int nSamples)
{
  VecVecFloat result;
...
int GameModerator::getDecisionMCTS(const VecInt& legalOptions, const VecVecFloat& vvf, VecFloat& sums)
{
  ...
  for (unsigned a = 0; a < legalOptions.size(); a++ ) {
    if (decverbose) cout << proc->operatorIndexToString(legalOptions[a]) << "; ";
  }
  if (decverbose) cout << "\n";
  for (unsigned i = 0; i < vvf.size(); i++) {
    for (unsigned a = 0; a < legalOptions.size(); a++ ) {
      if (decverbose) printf("%1.3f ",vvf[i][a]);
      sums[a] += vvf[i][a];
    }
    if (decverbose) printf("\n");
  } 
  if (decverbose) {
    printf("\n");
    for (unsigned a = 0; a < legalOptions.size(); a++ ) {
...
int GameModerator::chooseMoveMCTS(Processor* p, const VecVecVecKey& obs, const SetVecInt& infoset, const VecInt& legalOptions, const WorldState& initState, int nSamples)
{
  static const bool decverbose = false || gmverbose;
  ...
  if (decverbose) cout << "Optimal decision: " << bestAction << " (" << proc->operatorIndexToString(bestAction) << ")\n";
  //exit(0);
  //assert(false);
#ifdef INTERACTIVE
  int dummy;
...
int lastOpponentMove(const VecInt& sequence, unsigned pid, Processor* p, WorldState current)
{
  ...
  if (verbose) cout << "PID: " << pid << "\n";
  for (unsigned t = 1; t < sequence.size(); t++) {
    unsigned wt = current.getWhoseTurn();
    if (verbose) cout << "Action at time " << t << " is made by player " << wt << " (" << p->operatorIndexToString(sequence[t]) << ")\n";
    if (wt != pid && wt != 0) {
      result = t;
    } 
    p->shortApply(sequence[t],current,ncc); 
  }
  if (verbose) cout << "Last opponent move: " << result << "\n";
  return result; 
}

int GameModerator::chooseInferenceMove(Processor* p, unsigned pid, VecVecVecKey& obs, SetVecInt& infoset, const VecInt& legalOptions, WorldState& initState, int nSamples, int oppSamples)
{
  ...
  for (SetVecInt::const_iterator itr = infoset.begin(); itr != infoset.end(); ++i, ++itr) {  // For each node in the info set
    ...
    assert (ppid != 0);
    if (infverbose) cout << "GENERATING OPPONENT'S INFOSET\n";
...
// End
    if (infverbose) cout << "From node " << i << " bestMCTS option is " << bestOppAction << " " << p->operatorIndexToString(bestOppAction) << "\n";
    if (bestOppAction == lastDecision) {
      if (infverbose) cout << "This decision is consistent with this information set node\n";
      probs[i] = 1.0;
	//VecFloat GameModerator::mctsExplore(const VecInt& seq, const WorldState& initState, Processor* p, int nSamples)
      ...
      if (foundMatching && (sums[bestIndex] < .03 || (sums[matchingIndex] / sums[bestIndex]) > .9)) {
        if (gmverbose) cout << "This decision is close enough to the one observed to consider this information set node\n";
        if (gmverbose) cout << "sbi: " << sums[bestIndex] << " " << sums[matchingIndex] << "/" << sums[bestIndex] << " = " << sums[matchingIndex]/sums[bestIndex] << "\n";
        if (gmverbose) cout << "best index " << bestIndex << " matching index: " << matchingIndex  << "\n"; 
        probs[i] = 1.0;
          //VecFloat GameModerator::mctsExplore(const VecInt& seq, const WorldState& initState, Processor* p, int nSamples)
        decArray.push_back(mctsExplore(*itr, initState, p, nSamples));
        nConsistents++;
      } else {
        if (gmverbose) cout << "not consistent\n";
        probs[i] = 0.0;
        decArray.push_back(VecFloat(legalOptions.size(),0.0f));
      }
    }
    if (nConsistents >= (int)maxSize) break;
  ...
  if (gmverbose) cout << "INFOSET nodes that are CONSISTENT: " << nConsistents << "/" << infoset.size() << "\n"; 
  if (nConsistents == 0) return -2;
  VecFloat weightedEsts(legalOptions.size(),0.0f);
  for (unsigned j = 0; j < decArray.size(); j++) {
    assert (decArray[j].size() == legalOptions.size());
    if (medverbose) printf("%2.3f: ",probs[j]);
	...
	//cout << "Weighting: " << i << " " << j << " (" << p->operatorIndexToString(legalOptions[i]) << ") decArray: " << decArray[j][i] << " prob: " << probs[j] << "\n";
      ...
      if (medverbose) printf ("%2.3f ",decArray[j][i]);
    }
    if (medverbose) printf("\n");
  }
  if (medverbose) {
    printf("sums: ");
    for (unsigned i = 0; i < legalOptions.size(); i++) {
...
int GameModerator::chooseMCTSMove(Processor* p, unsigned pid, VecVecVecKey& obs, SetVecInt& infoSet, WorldState& initState)
{
  assert (!infoSet.empty());
  unsigned nOptions = -1; 
  bool firstOne = true;
  ...
  for (unsigned a = 0; a < nOptions; a++ ) {
    if (decverbose) cout << proc->operatorIndexToString(root.children[a].actionId) << "; ";
  }
  if (decverbose) cout << "\n";
  for (unsigned i = 0; i < actionValueEsts.size(); i++) {
    for (unsigned a = 0; a < nOptions; a++ ) {
      if (decverbose) printf("%1.3f ",actionValueEsts[i][a]);
...
void GameModerator::analyzeInfosetFailure(const VecInt& gameHistory, WorldState initState, WorldState current, Processor* p, unsigned k)
{
