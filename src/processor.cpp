...
void Processor::ensureCapacity(VecSetInt& vss, int neededIndex) {
    ...
    :maxPred(0), maxOperator(0), worldState(), gains(*parsedStructures->the_domain->gains), operators(*parsedStructures->the_domain->ops)
    {
    ...
    void Processor::processConstants(const const_symbol_table & constants)
    {
        ...
        for(StringToInt::const_iterator itr = typeNameIds.begin();itr != typeNameIds.end();++itr){
            ...
            if(superVerbose)
                cout << "There are " << typeTbl[itr->first].size() << " objects of type " << itr->first << " (id=" << itr->second << ")\n";
        ...
        for(operator_list::const_iterator itr = operators.begin();itr != operators.end();++itr){
            ...
            while(ptr >= 0){
                if(ptr >= (int)(((((((((nParams)))))))))){
                    ...
                    if(ncc.args[ptr] + 1 >= argMaxes[ptr]){
                        ncc.args[ptr] = -1;
                        ptr--;
                    }else{
                        ncc.args[ptr]++;
    ...
    void Processor::processPredicates(const pred_decl_list & predicates)
    {
        ...
        for(pred_decl_list::const_iterator itr = predicates.begin();itr != predicates.end();++i, ++itr){
            ...
            for(unsigned j = 0;j < argTypes.size();j++){
                ...
                if(superVerbose)
                    cout << "Parameter " << j << " for predicate " << predName << "(id " << i << ") has type " << argTypes[j] << " (id " << typeNameIds[argTypes[j]] << ") which has cardinality " << typeCardinalities[typeNameIds[argTypes[j]]] << "\n";
        ...
        for(StringToBounds::iterator itr = factsOffsets.begin();itr != factsOffsets.end();++itr){
            ...
            itr->second.upper += currentOffset - 1;
            if(superVerbose)
                cout << "PredName: " << itr->first << " min: " << itr->second.lower << " max: " << itr->second.upper << endl;
