#ifndef __CONDITIONCONTROLLER
#define __CONDITIONCONTROLLER
...
class ConditionController : public VisitController {
public:
	bool achievedFixedPoint;
	//int* state;
	StringToInt params;
        stack< bool > satConditions;
	bool truthValue;
	vector<string> argStrings;
        VAL::Processor* proc;
        WorldState& ws;
	ConditionController(VAL::Processor* proc_);
 	//void set(int factNum);
	void resetFixedPointCheck();
	void foundNewTrueFact();
	void resetParamList();
	bool hasFixedPoint();
 	//void copyState(int* s);
	virtual ~ConditionController() {};
