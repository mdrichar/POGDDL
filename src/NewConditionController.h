#ifndef __NEWCONDITIONCONTROLLER
#define __NEWCONDITIONCONTROLLER
...
class NewConditionController : public VisitController {
private:
...
public:
	...
	NewConditionController(VAL::Processor* proc_, WorldState& worldState);
        void setArgSize(unsigned nArgs) {args.resize(nArgs);}
 	void set(int factNum);
	void unset(int factNum);
	void resetParamList();
 	//void copyState(int* s);
   	//void clearEffects();
	FunctionKey getGroundedProp(proposition* p, const VecInt& args);
	bool passesScreeningChecks(operator_* op, int argNum);
	bool passesQFiedScreeningChecks(qfied_goal* g, int argNum);

	virtual ~NewConditionController() {};
	...
	virtual void visit_special_val_expr(special_val_expr *) {assert(false);};
	virtual void visit_violation_term(violation_term *) {assert(false);};
	...
	virtual void visit_constraint_goal(constraint_goal *) {assert(false);};
	...
	virtual void visit_observation(observation *) ;
	virtual void visit_gain(gain_def *) ;
	...
	virtual void visit_gain(const gain_def* g) {visit_gain(const_cast<gain_def*>(g));};
