...
int Processor::factNumLookupByArgIndex(const string& predName, const VecInt& indices, const VecInt& args)
{
  ...
  if (intArgs.size() < nArgs) intArgs.resize(nArgs);
...
void Processor::ensureCapacity(VecSetInt& vss, int neededIndex)
{
  ...
  if ((int)vss.size() <= neededIndex) vss.resize(neededIndex+1);
}
  ...
  for (unsigned i = 1; i < vi.size(); i++) {
    if (vi[i] == UVAL) {
	  ...
	  break;
        case REVELATION:
          revId = vi[0];
          assert (revId < revParamTypeTbl.size());
          revArgTypes = revParamTypeTbl[revId];
...
Processor::Processor(VAL::analysis* parsedStructures)
  : maxPred(0), maxOperator(0), worldState(), gains(*parsedStructures->the_domain->gains), 
  operators(*parsedStructures->the_domain->ops)
{
...
void Processor::processConstants(const const_symbol_table& constants)
{
  for (const_symbol_table::const_iterator itr = constants.begin(); itr != constants.end(); ++itr) {
    ...
    if (superVerbose && (*itr).second->type) cout << ":" << (*itr).second->type->getName(); // constant is typed
    //cout <<  endl;    
  ...
  for (StringToInt::const_iterator itr = typeNameIds.begin(); itr != typeNameIds.end(); ++itr) {
    ...
    if (superVerbose) cout << "There are " << typeTbl[itr->first].size() << " objects of type " << itr->first << " (id=" << itr->second << ")\n";
  }
}
// payoffs
...
void Processor::computePayoffs(WorldState& ws)
{
  ...
  for (unsigned i = 0; i < payoffs.size(); i++) payoffs[i] = 0;
  int j = 0;
...
VecInt Processor::legalOperators(WorldState& partialWorld)
{
  ...
  for (operator_list::const_iterator itr = operators.begin(); itr != operators.end(); ++itr) {
    ...
    if (verbose) cout << "CHECKING ARG LEGALITY: " << opName << "\n";
    VecInt& argMaxes = (*itr)->argMaxes;
    ncc.args = VecInt ((*itr)->getArgSizeNeeded(),-1);
    int nParams = (int)(*itr)->parameters->size();
    if (verbose) cout << "OP: " << opName << " argssize: " << ncc.args.size() << "\n";
    if (!ncc.passesScreeningChecks(*itr,-1)) {
      if (verbose) cout << "Failed parameter-independent screening checks\n";
      continue;
    }
    int ptr = 0;
    while (ptr >= 0) {
...
void Processor::processOperators()
{
  ...
  for (operator_list::const_iterator itr = operators.begin(); itr != operators.end(); ++opId, ++itr) {
    ...
    for (var_symbol_list::const_iterator sitr = (*itr)->parameters->begin(); sitr != (*itr)->parameters->end(); ++sitr) {
        if (superVerbose) cout << (*sitr)->type->getName() << " ";
  ...
  for (StringToBounds::iterator itr = opsOffsets.begin(); itr != opsOffsets.end(); ++itr) {
    ...
    if (superVerbose) cout << "OpName: " << itr->first << " min: " << itr->second.lower << " max: " << itr->second.upper << endl;
    maxOperator = itr->second.upper;
    currentOffset += differential;
  }

  for (StringToInt::const_iterator itr = revHeadTbl.begin(); itr != revHeadTbl.end(); ++itr) {
    if (superVerbose) cout << itr->first << ":" << itr->second << "\n";
    assert (itr->second < (int)revParamTypeTbl.size());
    assert (itr->second < (int)revParamTypeCardTbl.size());
    for (unsigned j = 0; j < revParamTypeTbl[itr->second].size(); j++) {
      if (superVerbose) cout << "Param " << j << " has id " << revParamTypeTbl[itr->second][j] << " and card " << revParamTypeCardTbl[itr->second][j] << "\n";
    }
  }
...
void Processor::processConstants(const const_symbol_list& constants)
{
  for (const_symbol_list::const_iterator itr = constants.begin(); itr != constants.end(); ++itr) {
    cout << "constant in list: " << (*itr)->getName(); 
    if ((*itr)->type) cout << ":" << (*itr)->type->getName();
    cout <<  endl;    

  }
}

void Processor::processConstants(domain* p)
{
...
void Processor::processPredicates(const pred_decl_list& predicates)
{
  ...
  for (pred_decl_list::const_iterator itr = predicates.begin(); itr != predicates.end(); ++i, ++itr) {
    ...
    if (superVerbose) cout << "PredHead " << predName << " = " << i << "\n";
	...
	if (superVerbose) cout << "Parameter " << j << " for predicate " << predName << "(id " << i << ") has type " << argTypes[j] << " (id " << typeNameIds[argTypes[j]] << ") which has cardinality " << typeCardinalities[typeNameIds[argTypes[j]]] << "\n";
    }
    offsets[predName] = 0;
    ...
    if (!mults.empty()) {
      ...
      if (superVerbose) cout << "predicate " << predName << ": ";   // Name of the predicate
      int index = 0;
      for (var_symbol_list::const_iterator sitr = (*itr)->args->begin(); sitr != (*itr)->args->end(); ++sitr) {
        if (superVerbose) cout << (*sitr)->type->getName() << "(" << mults[index] << ") ";
        ++index;
      }
      if (superVerbose) cout << " total: " << counts[predName] << "\n";
  ...
  for (StringToBounds::iterator itr = factsOffsets.begin(); itr != factsOffsets.end(); ++itr) {
    ...
    itr->second.lower += currentOffset;
    itr->second.upper += currentOffset - 1;
    if (superVerbose) cout << "PredName: " << itr->first << " min: " << itr->second.lower << " max: " << itr->second.upper << endl;
...
void Processor::processTypes(const pddl_type_symbol_table& types)
{
  ...
  for (pddl_type_symbol_table::const_iterator itr = types.begin(); itr != types.end(); ++i, ++itr) {
	if (superVerbose) cout << (*itr).first << " " << i << "\n";
        typeNameIds[(*itr).first] = i;
	typeIdNames.push_back((*itr).first);
  }
}

void Processor::preWrite()
{
...
void Processor::generateFacts(const string& pred, const VecStr& argTypes, VecInt& args, unsigned level)
{
  ...
  if (level == argTypes.size() ) {
    if (verbose) cout << pred << " ";
    if (verbose) cout << ": " << pred << "(";
    for (unsigned i = 0; i < args.size(); i++) {
      if (verbose) if (i) cout << ", " ;
      if (verbose) cout << invObjectTbl[argTypes[i]][args[i]] ;
    }
    if (verbose) cout << "): "  << getIndex(pred,args) << "\n" ;
  } else {
...
void Processor::computeDerivedPredClosures(derivations_list& drvs) 
{
  ...
  ConditionController cc(this);
  do {
    if (superVerbose) cout << "Computing Deriveds" << std::endl;
  ...
  if (superVerbose) cout << "Finally true facts: " << "\n";
  if (superVerbose) for (unsigned i = 0; i < maxPred; i++) {
    printState();
  }
}
...
{
  if (superVerbose) cout << "INITIAL STATE" << endl;
...
void Processor::generateOffsets()
{
  ...
  for (StringToInt::const_iterator itr = counts.begin(); itr != counts.end(); ++itr) {
    ...
    if (superVerbose) cout << "Offset for " << itr->first << ": " << sumTotal << "\n";
    sumTotal += itr->second; 
  }
}
...
const StringToBoundsEntry Processor::getPredName(const StringToBounds& map, unsigned index)
{
  ...
  while (itr->second.upper < index && itr != map.end()) ++itr;
  assert (itr != map.end());
  return *itr;
}
...
StringToIntEntry Processor::getPredName(int index)
{
  ...
  while (countsItr != counts.end()) {
    if (countsItr->second + currentOffsetItr->second > index) break;
    ++currentOffsetItr;
    ++countsItr;
  } 
...
void Processor::decodeOperator(int index, string& oName, VecInt& oArgs, bool resize)
{
  ...
  if (resize) oArgs.resize(mults.size());
...
int Processor::getIndex(const StringToBounds& offsets, const PredicateMultTable& multTbl, const string& name, const VecInt& args)
{
  ...
  const Bounds& bounds = bitr->second;
  assert (args.size() >= mults.size()); // args may have have some additional filler that was used for the quantified subterms
...
{
  if (readObs) kb.push_back(ws.obsListByPlyr); // Need to do this first because applyUpdates will clear out the observations
  ws.applyUpdates();
}
...
{
  if (level == argTypes.size() ) {
    if (superVerbose) cout << opName << "(";
    if (superVerbose) for (unsigned i = 0; i < level; i++) {
      if (i) cout << ", ";
      cout << invObjectTbl[argTypes[i]][args[i]];
    }
    if (superVerbose) cout << ") <=> ";
    const Bounds& bounds = opsOffsets[opName];
    if (superVerbose) cout << getOperatorIndex(bounds.lower,opsMults[opName],args);
    if (superVerbose) cout << "\n";
  } else {
    unsigned thisMax = typeTbl[argTypes[level]].size();
...
{
  if (superVerbose) cout << "ENUMERATE OPERATORS" << endl;
...
void Processor::printState()
{
  ...
  unsigned p = 0;
  if (nRoles > 2) p++; // Don't print out for chance (this is a kludge)
  for (; p < nRoles; p++) {
...
{
  return revHeadTbl.find(name) != revHeadTbl.end();
}

unsigned Processor::newRevelationHeadId(const string& name, unsigned nArgs) {
  assert (revHeadTbl.size() == revParamTypeTbl.size());
  unsigned result = revHeadTbl.size();
  revParamTypeTbl.push_back(VecInt(nArgs));
  revHeadTbl[name] = result;
  return result;
}
...
VecInt Processor::getPartialOperator(const VecInt& observation, const VecInt& argMapper)
{
  ...
  for (unsigned i = 0; i < argMapper.size() - 1; i++) {
    ...
    if ((argMapper[i] == UVAL) ^ (observation[i+1] == UVAL)) return VecInt();
    if (argMapper[i] < 0) {
      if (argMapper[i] != observation[i+1]) return VecInt(); // If one of the arguments in the observation is a constant, it must match
...
void Processor::sandbox2(const VecVecVecKey& kb, const VecSetVecInt& opsByRev) {
  cout << "SANDBOX2\n" << printPartialState(initialWorld) << "\n";
  InfoSetGenerator isg(this,kb,initialWorld);
  isg.generate(1,10);
  //ActionGraph ag (this,kb,opsByRev,initialWorld);
  //ag.initializeGraph();
