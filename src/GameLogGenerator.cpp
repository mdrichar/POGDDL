...
void GameLogGenerator::generateGames(const string& logString, const string& domainString, const string& problemString,
		unsigned nGamesToPlay) {
...
GameLogReader::GameLogReader(const string& domainString, const string& problemString, const string& logString,
		WorldStateFormatter* formatter) {
	this->logString = logString;
	this->an_analysis = GameParser::parseGame(domainString, problemString);
	this->p = new VAL::Processor(an_analysis);
	this->formatter = FormatterFactory::createFormatter(p);
	this->formatter->setProcessor(this->p);
...
void GameLogReader::sumPayoffsOverManyGames() {
	...
	while (!gameLogInFile.eof()) {
		...
		if (!oneGameStream.eof()) {
			...
			if (!foundAcceptable) {
				...
				cout << p->getFormattedState(gameHistory, currentGameState, p->kb) << endl;

				static const bool testInfoSets = true;
				if (testInfoSets) {
					...
					cout << currentLineNumber << " Infoset size using dfs: " << infoset.size() << "  First action: "
							<< gameHistory[1] << endl;
					// Accumulate
					...
					cout << currentLineNumber << " Infoset size using cbs: " << infosetCBS.size() << "  First action: "
							<< gameHistory[1] << endl;
					// Accumulate
	...
	cout << "#games: " << gameCount << " avg.dpth: " << avgMoveCount << " DFS found: " << overallSolutionCountDFS
			<< " ops: " << overallOperationCountDFS << " efficiency: " << overallEfficiencyDFS << " CBS found: "
			<< overallSolutionCountCBS << " ops: " << overallOperationCountCBS << " efficiency: "
			<< overallEfficiencyCBS << " " << logString << endl;
}
