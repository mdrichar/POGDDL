...
int main(int argc, char* argv[])
{
  ...
  printf ("(define (problem mast%d-%d)\n",slotCount,cardCount);
  printf ("(:domain masterind)\n"); 
  printf ("(:objects)\n     "); 
  for (unsigned slotIndex = 1; slotIndex <= slotCount; slotIndex++) {
    printf("p%02d " ,slotIndex);
  }
  printf (" - position\n     ");
  for (unsigned cardIndex = 1; cardIndex <= cardCount; cardIndex++) {
    printf("n%02d ",cardIndex);
  }
  printf (" - number)\n");
  printf ("(:init\n");
  printf ("     (inphase initializing)\n");
  printf ("     (current p00)\n");
  printf ("     (last p%02d)\n",slotCount - 1);
  printf ("     (last n%02d)\n",slotCount);
  printf ("     (whoseturn p1)\n");
  printf ("     (opponentof p1 p2)\n");
  printf ("     (opponentof p2 p1)\n");
  printf ("     (= (nturns) 0)\n");
  for (unsigned cardIndex = 0; cardIndex < cardCount; cardIndex++) {
    printf("     (next  p%02d p%02d)\n",cardIndex);
  }
  for (unsigned slotIndex = 1; slotIndex < slotCount; slotIndex++) {
  ...
  for (unsigned cardIndex = 1; cardIndex < cardCount; cardIndex++) {
    printf("     (successor-c u%02d u%02d)\n", cardIndex + 1, cardIndex);
  }
  printf("))\n");
  return EXIT_SUCCESS;
}
