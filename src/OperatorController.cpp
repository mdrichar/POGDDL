...
void OperatorController::considerOptimizations(simple_goal* g) {
	if (!preconditionMode)
		return; // Don't process the conditions for conditional effects
	if (goalLevel <= 1) {
		int maxArg = g->getProp()->lastParamUsed();
		if (maxArg < 0) { // all constants
			int factNum = proc->factNumLookupByArgIndex(
					g->getProp()->getHead()->getName(), g->getProp()->argMap,
					VecInt());
...
OperatorController::OperatorController(VAL::Processor* proc_) :
		proc(proc_) {
	assert(proc);
	goalLevel = 0;
	negationLevel = 0;
...
void OperatorController::assignParameterIndex(const string& name) {
	...
	if (!(opArgsMap.find(name) == opArgsMap.end())) {
		if (VERBOSE)
			cout << "Repeat arg: " << name << " " << opArgsMap[name] << "\n";
...
void OperatorController::visit_operator_(operator_* op) {
	...
	int i = 0;
	for (var_symbol_list::const_iterator sitr = op->parameters->begin();
			sitr != op->parameters->end(); ++i, ++sitr) {
...
void OperatorController::visit_gain(gain_def *g) {
	...
	for (var_symbol_list::const_iterator sitr = g->getArgs()->begin();
			sitr != g->getArgs()->end(); ++sitr) {
		assignParameterIndex((*sitr)->getName());
		//op->argMaxes[i] = proc->typeTbl[(*sitr)->type->getName()].size();
	}
...
void OperatorController::visit_simple_goal(simple_goal* g) {
	...
	int i = 0;
	if (VERBOSE)
		cout << "Prop: " << g->getProp()->getHead()->getName() << " ";
	for (parameter_symbol_list::const_iterator paramItr = psl.begin();
			paramItr != psl.end(); ++paramItr, ++i) {
	...
	g->getProp()->setHeadId(
			proc->predHeadTbl[g->getProp()->getHead()->getName()]);
	considerOptimizations(g);
	considerQFiedGoalOptimizations(g);
	if (VERBOSE)
		cout << "  Setting up argMap for " << g->getProp()->getHead()->getName()
				<< "(";
	if (VERBOSE)
...
void OperatorController::visit_qfied_goal(qfied_goal* g) {
	...
	int ptr = 0;
	for (var_symbol_list::const_iterator sitr = vsl.begin(); sitr != vsl.end();
			++sitr) {
		if (VERBOSE)
		...
		if (argEntry == opArgsMap.end()) {
			...
			assert(false);
			// This means that one of the variables being quantified over
...
void OperatorController::visit_effect_lists(effect_lists* e) {
	...
	for (pc_list<simple_effect*>::iterator itr = addEffects.begin();
			itr != addEffects.end(); ++itr) {
		(*itr)->visit(this);
	}
	pc_list<simple_effect*>& delEffects = e->del_effects;
	for (pc_list<simple_effect*>::iterator itr = delEffects.begin();
			itr != delEffects.end(); ++itr) {
		(*itr)->visit(this);
	}
	pc_list<cond_effect*>& condEffects = e->cond_effects;
	int i = 0;
	for (pc_list<cond_effect*>::iterator itr = condEffects.begin();
			itr != condEffects.end(); ++itr, ++i) {
		(*itr)->visit(this);
	}
	pc_list<assignment*>& assignEffects = e->assign_effects;
	for (pc_list<assignment*>::iterator itr = assignEffects.begin();
			itr != assignEffects.end(); ++itr) {
		(*itr)->visit(this);
	}
	pc_list<cond_effect*>& condAssgnEffects = e->cond_assign_effects;
	for (pc_list<cond_effect*>::iterator itr = condAssgnEffects.begin();
			itr != condAssgnEffects.end(); ++itr) {
		assert(false);
	...
	i = 0;
	for (pc_list<observation*>::iterator itr = observations.begin();
			itr != observations.end(); ++itr, ++i) {
		(*itr)->visit(this);
	}

	pc_list<forall_effect*>& forallEffects = e->forall_effects;
	for (pc_list<forall_effect*>::iterator itr = forallEffects.begin();
			itr != forallEffects.end(); ++itr) {
		(*itr)->visit(this);
	}
...
void OperatorController::visit_simple_effect(simple_effect* e) {
	...
	int i = 0;
	if (VERBOSE)
		cout << "EProp: " << e->getProp()->getHead()->getName() << " ";
	for (parameter_symbol_list::const_iterator paramItr = psl.begin();
			paramItr != psl.end(); ++paramItr, ++i) {
		if ((*paramItr)->isConstant()) {
			argMap[i] = -1 - proc->objectTbl[(*paramItr)->getName()];
	...
	if (VERBOSE)
		cout << "MUTABLE: " << e->prop->getHead()->getName() << "\n";
	if (VERBOSE)
...
void OperatorController::visit_observation(observation *r) {
	...
	for (recipients::const_iterator itr = r->receivers->begin();
			itr != r->receivers->end(); ++itr) {
		//cout << (*itr)->receivers->size() << " ";
		for (parameter_symbol_list::const_iterator pitr =
				(*itr)->receivers->begin(); pitr != (*itr)->receivers->end();
				++pitr) {
			//cout << ":";
...
void OperatorController::processRevMsg(proposition* p) {
	...
	for (parameter_symbol_list::const_iterator paramItr = obsArgList.begin();
			paramItr != obsArgList.end(); ++paramItr, ++i) {
		if ((*paramItr)->isConstant()) {
			if ((*paramItr)->getName() == "u") { // Special treatment for the "unknown value"
				argMap[i] = UVAL;
				revMapper[i] = UVAL;
			} else {
				argMap[i] = -1 - proc->objectTbl[(*paramItr)->getName()];
				...
				int typeId =
						proc->typeNameIds[proc->objectTypes[(*paramItr)->getName()]];
				assert(
						revParamTypeIds[i] == UVAL || revParamTypeIds[i] == typeId);
			...
			assert(revParamTypeIds[i] == UVAL || revParamTypeIds[i] == typeId);
			revParamTypeIds[i] = typeId;
			revParamTypeCards[i] = proc->typeCardinalities[typeId];
		}
	}
