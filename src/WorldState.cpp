...
WorldState::WorldState(const WorldState& other) :
		maxPred(other.maxPred), nRoles(other.nRoles), fluentVals(other.fluentVals), fluentUpdates(other.fluentUpdates), obsListByPlyr(
				other.obsListByPlyr) {
	//int intsize = intLength(maxPred);
	//stateBits = new int[intsize];
	//knownFlags = new int[intsize];
...
void WorldState::applyUpdates() {
	...
	for (FluentTable::const_iterator itr = fluentUpdates.begin(); itr != fluentUpdates.end(); ++itr) {
		assert(fluentVals.find(itr->first) != fluentVals.end());
		// This is not technically a problem; it's ok to assign a value to an undefined numeric constant; I'm just putting this here because I'm currently not expecting any of these
...
int WorldState::getWhoseTurn() const {
	...
	for (unsigned i = 0; i < roleIds.size(); i++) {
		...
		assert(ts != UNKNOWN);
		// A player should always know if it's his turn
		if (ts == KNOWN_TRUE) {
			playerId = i;
...
// Return the index of the fact of the predName propType, where the ith argument is args[inidcies[i]]
// for variables and -1-indices[i] for constants
NumScalar WorldState::fastFuncLookupByArgIndex(VecInt& intArgs, unsigned funcId, const VecInt& indices,
		const VecInt& args) {
	//static VecInt intArgs;
...
bool WorldState::getTruthValue(int headId, int arg1) {
	VecInt args;
	args.push_back(arg1);
	int index = getIndex(args, headId);
	return this->getTruthValue(index);
}

bool WorldState::getTruthValue(int headId, int arg1, int arg2) {
	VecInt args;
	args.push_back(arg1);
	args.push_back(arg2);
	int index = getIndex(args, headId);
...
NumScalar WorldState::getFluentValue(int headId) {
	VecInt args;
	args.push_back(headId);
	return this->getValue(args);
}

NumScalar WorldState::getFluentValue(int headId, int arg1) {
	VecInt args;
	args.push_back(headId);
	args.push_back(arg1);
	return this->getValue(args);
