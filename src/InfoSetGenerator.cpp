...
SetVecInt InfoSetGenerator::generate(unsigned k, unsigned maxSize)
{
  ...
  if (verbose) cout << "Total combinations: " << totalCombinations << "\n";
  if (verbose) cout << "ACTION GRAPH\n";
  if (verbose) cout << initialAgStringBrief << "\n";
  if (totalCombinations == 1 && !overflow) {
    if (verbose) cout << "ONE OPTION\n";
    ...
    if (verbose) cout << "RECURSIVELY GENERATE" << totalCombinations << std::endl;
...
void InfoSetGenerator::generateNaive(VecInt& sequence, WorldState& currentWorld, unsigned t, SetVecInt& results, double startTime, double timeLimit, unsigned maxSize)
{
  if (results.size() >= maxSize) return;  
  if (t >= sequence.size()) {
...
void InfoSetGenerator::generateRec(SetVecInt& result)
{
  ...
  assert (!ag.allActionSetsAreSingletons()); // Otherwise, this method should not have been called
